...
static int update_userlimit_cpufreq_min(int cid, int value);
static struct task_struct *trusty_task = NULL;
...
static int pmic_key_reset_func(void *data)
{
	msleep(50);
	update_userlimit_cpufreq_min(0, 450000);
	update_userlimit_cpufreq_min(1, 450000);
	printk("pmic_key_reset_func end!\n");
	return 0;
}
...
static int __init pmic_key_cpu_ctrl_init(void)
{
	int cpu_num = 0;
	int num = 0;
	int cpu;
...
//#endif /*OPLUS_BUG_STABILITY*/
	update_userlimit_cpufreq_min(0, 2000000);
	update_userlimit_cpufreq_min(1, 2000000);
	printk("mtk_pmic_keys_irq_handler_thread started !\n");

	trusty_task = kthread_create(pmic_key_reset_func, NULL,"pmic_key_reset_func");
	if (IS_ERR(trusty_task))
		printk("[%s] Unable to start on : stress_trusty_thread",
			__func__);
	else {
		printk("[%s] Start stress_trusty_thread on cpu ",
			__func__);
