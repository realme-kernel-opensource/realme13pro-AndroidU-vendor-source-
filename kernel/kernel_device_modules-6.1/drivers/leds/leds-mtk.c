...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
	...
	pr_info("parse led: %s, led-mode: %d", mdev->conf.cdev.name, mdev->conf.mode);
	...
	pr_info("parse led: %s, num: %d, connector id: %d, max: %d, min: %d, max_hw: %d, min_hw: %d, brightness: %d, led-mode: %d",
		mdev->conf.cdev.name,
		leds_info->lens,
