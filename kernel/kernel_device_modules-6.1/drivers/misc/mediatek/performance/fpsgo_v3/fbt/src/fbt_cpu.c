...
static int rl_deq_length_thres;
...
module_param(rl_deq_length_thres, int, 0644);
...
void fbt_set_render_boost_attr(struct render_info *thr)
{
	...
	if (pid_attr.rl_deq_length_thres_by_pid != BY_PID_DEFAULT_VAL)
		render_attr->rl_deq_length_thres_by_pid =
...
void fbt_update_freq_qos_min(int policy_id, unsigned int freq)
{
	...
	fpsgo_systrace_c_fbt(-100, 0, freq_qos_cap_temp, "freq_qos_cap_temp");
	...
	long loading, unsigned int target_fpks, int cooler_on)
{
	...
	if (!fps_margin && rl_deq_length_thres_final > 0 && thread_info->dequeue_length >=
		(unsigned long long) rl_deq_length_thres_final) {
		if (t2 < boost_info->last_target_time_ns)
			t2 = boost_info->last_target_time_ns;
	}

	if (rl_deq_length_thres > 0 && thread_info->dequeue_length >= (unsigned long long) rl_deq_length_thres) {
		if (t2 < boost_info->last_target_time_ns)
			t2 = boost_info->last_target_time_ns;
	}
		...
		const char *buf, size_t count)
{
	...
	} else if (!strcmp(cmd, "rl_deq_length_thres")) {
		if ((val <= 100000000 && val >= 0) && action == 's')
			boost_attr->rl_deq_length_thres_by_pid = val;
		else if (val == BY_PID_DEFAULT_VAL && action == 'u')
