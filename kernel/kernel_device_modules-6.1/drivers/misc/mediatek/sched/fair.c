		 ...
		 struct task_struct *p, int dst_cpu)
{
	...
	unsigned int count_cpu = cpumask_weight(pd_cpus);
	unsigned long util_thresh = 0;
	unsigned long capacity = arch_scale_cpu_capacity(cpumask_first(pd_cpus));
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_VT_CAP)
		avg_nr_running += cpu_rq(cpu)->nr_running;
#endif
	}
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_VT_CAP)
	if (eas_opt_enable && (util_thresh_percent[gear_idx] != 100) && count_cpu) {
		unsigned long max_util_base = eenv->pds_max_util[gear_idx][0];
		unsigned long max_util_cur = eenv->pds_max_util[gear_idx][dst_idx];

		util_thresh = capacity * util_thresh_cvt[gear_idx] >> SCHED_CAPACITY_SHIFT;
		avg_nr_running = mult_frac(avg_nr_running, 1, count_cpu);
		eenv->pds_max_util[gear_idx][dst_idx] = (util_thresh < max_util_cur) ?
			(util_thresh + ((avg_nr_running * (max_util_cur - util_thresh)*nr_oplus_cap_multiple[gear_idx]) >> SCHED_CAPACITY_SHIFT)) : max_util_cur;
		if (dst_cpu != -1) {
			eenv->pds_max_util[gear_idx][0] = (util_thresh < max_util_base) ?
				(util_thresh + ((avg_nr_running * (max_util_base - util_thresh)*nr_oplus_cap_multiple[gear_idx]) >> SCHED_CAPACITY_SHIFT)) : max_util_base;
		}
		if (unlikely(eas_opt_debug_enable))
			trace_printk("[eas_opt]: cluster_id: %d, capacity: %lu, util_thresh: %lu, avg_nr_running: %d, "
					"origin_max_util_base: %lu, max_util_base: %lu, origin_max_util_cur: %lu, max_util_cur: %lu, util_thresh_percent: %d\n",
					gear_idx, capacity, util_thresh, avg_nr_running, max_util_base, eenv->pds_max_util[gear_idx][0],
					max_util_cur, eenv->pds_max_util[gear_idx][1], util_thresh_percent[gear_idx]);
	}
#endif
