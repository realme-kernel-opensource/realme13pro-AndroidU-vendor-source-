...
static void mkp_task_newtask(void *ignore, struct task_struct *task, unsigned long clone_flags)
{
	int ret = -1;
	struct cred_sbuf_content c;
	const struct cred *temp_cred = NULL;
	...
	c.csc.uid.val = temp_cred->uid.val;
	c.csc.gid.val = temp_cred->gid.val;
	c.csc.euid.val = temp_cred->euid.val;
	c.csc.egid.val = temp_cred->egid.val;
	c.csc.fsuid.val = temp_cred->fsuid.val;
	c.csc.fsgid.val = temp_cred->fsgid.val;
	c.csc.security = temp_cred->security;
	ret = mkp_update_sharebuf_4_argu(MKP_POLICY_TASK_CRED, g_ro_cred_handle,
			(unsigned long)task->pid,
			c.args[0], c.args[1], c.args[2], c.args[3]);
