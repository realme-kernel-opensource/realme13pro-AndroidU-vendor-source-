...
bool cmdq_irq_enable_mbox(struct device *dev)
{
	if (!dev)
		return false;

	return pm_runtime_get_if_in_use(dev) > 0 ? true : false;
}

bool cmdq_irq_disable_mbox(struct device *dev)
{
	if (!dev)
		return false;
...
struct cmdq_util_platform_fp platform_fp = {
	...
	.irq_enable_mbox = cmdq_irq_enable_mbox,
	.irq_disable_mbox = cmdq_irq_disable_mbox,
};
