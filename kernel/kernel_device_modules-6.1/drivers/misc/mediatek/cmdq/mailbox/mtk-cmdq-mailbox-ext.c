...
void cmdq_dump_usage(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		cmdq_msg(
			"%s: usage:%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
			__func__,
			...
			usage[20], usage[21], usage[22], usage[23], usage[24],
			usage[25], usage[26], usage[27], usage[28], usage[29],
			usage[30], usage[31]);
	}
}
...
static irqreturn_t cmdq_irq_handler(int irq, void *dev)
{
	...
	u32 end_cnt = 0;
#endif
	if(cmdq_util_controller->irq_enable_mbox) {
		if(cmdq_util_controller->irq_enable_mbox(cmdq->mbox.dev))
			cmdq->cmdq_irq_self_ena_mbox = true;
		else
			cmdq_err("enable irq mbox fail");
	}
...
#endif
	if(cmdq_util_controller->irq_disable_mbox)
		cmdq_util_controller->irq_disable_mbox(cmdq->mbox.dev);

	cmdq->cmdq_irq_self_ena_mbox = false;
