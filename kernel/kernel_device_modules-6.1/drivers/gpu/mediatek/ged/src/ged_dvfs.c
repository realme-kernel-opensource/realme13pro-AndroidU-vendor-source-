	...
	unsigned long ui32NewFreq, GED_DVFS_COMMIT_TYPE eCommitType)
{
	...
	if (ged_dvfs_gpu_freq_commit_fp != NULL) {
		...
		trace_perfetto_tracing_mark_write("gpu_freq_ceil",
							(long long) ged_get_freq_by_idx(ui32CeilingID) / 1000);
		trace_perfetto_tracing_mark_write("gpu_freq_floor",
							(long long) ged_get_freq_by_idx(ui32FloorID) / 1000);
		trace_perfetto_tracing_mark_write("limitter_ceil", ged_get_cur_limiter_ceil());
		trace_perfetto_tracing_mark_write("limitter_floor", ged_get_cur_limiter_floor());
		trace_perfetto_tracing_mark_write("commit_type", eCommitType);

		if (dcs_get_adjust_support() % 2 != 0){
			trace_tracing_mark_write(5566, "preserve", g_force_disable_dcs);
			trace_perfetto_tracing_mark_write("preserve", g_force_disable_dcs);
		}
	...
	unsigned long topNewFreqID)
{
	...
	trace_perfetto_tracing_mark_write("gpu_freq_floor", (long long) ged_get_freq_by_idx(ui32FloorID) / 1000);
	trace_perfetto_tracing_mark_write("limitter_ceil", ged_get_cur_limiter_ceil());
	trace_perfetto_tracing_mark_write("limitter_floor", ged_get_cur_limiter_floor());
	trace_perfetto_tracing_mark_write("commit_type", eCommitType);
	...
	int target_fps_margin, unsigned int force_fallback)
{
	...
	trace_perfetto_tracing_mark_write("t_gpu", (long long) t_gpu * 100);
	trace_perfetto_tracing_mark_write("t_gpu_target", (long long) t_gpu_target * 100);
		...
		bool bRefreshed)
{
	...
	if (g_gpu_timer_based_emu) {
		...
		trace_perfetto_tracing_mark_write("t_gpu", t_gpu);
		trace_perfetto_tracing_mark_write("t_gpu_target", t_gpu_target);

		/* set cur freq back to before api boost if exit api boost */
