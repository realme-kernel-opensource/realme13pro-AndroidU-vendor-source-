...
char limit_task[TASK_COMM_LEN] = { 0 };
pid_t limit_pid;
int limit_freq = 0;

static ssize_t oplus_frequency_limit_show(struct kobject *kobj,
                struct kobj_attribute *attr,
                char *buf)
{
        return scnprintf(buf, PAGE_SIZE, "Task name:  %s\nTask pid: %d\nLimit Freq: %d\n",
                limit_task, limit_pid, limit_freq);
}

static KOBJ_ATTR_RO(oplus_frequency_limit);
//-----------------------------------------------------------------------------
		...
		const char *buf, size_t count)
{
	...
	if ((count > 0) && (count < GED_SYSFS_MAX_BUFF_SIZE)) {
		pr_info("custom_upbound_gpu_freq_store: %s(%d) limit gpu freq %s", current->comm, current->pid, buf);
		if (scnprintf(acBuffer, GED_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			if (kstrtoint(acBuffer, 0, &i32Value) == 0) {
				strcpy(limit_task, current->comm);
				limit_pid = current->pid;
				limit_freq = i32Value;
...
GED_ERROR ged_hal_init(void)
{
	...
	if (unlikely(err != GED_OK)) {
        ...
        err = ged_sysfs_create_file(hal_kobj,
                &kobj_attr_oplus_frequency_limit);
        if (unlikely(err != GED_OK)) {
                GED_LOGE(
                        "Failed to create oplus_frequency_limit entry!\n");
                goto ERROR;
        }
