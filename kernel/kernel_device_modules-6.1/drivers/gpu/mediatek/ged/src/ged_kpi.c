...
static void ged_kpi_work_cb(struct work_struct *psWork)
{
	...
	case GED_TIMESTAMP_TYPE_P:
		...
		if (psKPI) {
			...
			trace_perfetto_tracing_mark_write("t_pre_fence_delay", pre_fence_delay);
		} else {
	...
	case GED_SET_TARGET_FPS:
		...
		trace_tracing_mark_write(5566, "target_fps_fpsgo",
			(target_FPS&0x000000ff));
		trace_perfetto_tracing_mark_write("target_fps_fpsgo", (target_FPS&0x000000ff));
	...
	case GED_SET_PANEL_REFRESH_RATE:
		...
		if (target_FPS > 0 &&
			...
			trace_perfetto_tracing_mark_write("target_fps_panel", target_FPS);
		}
		break;
...
void ged_kpi_update_sysram_uncompleted_tgpu(struct ged_sysram_info *info)
{
	...
	trace_perfetto_tracing_mark_write("last_tgpu_uncompleted", info->last_tgpu_uncompleted);
}
