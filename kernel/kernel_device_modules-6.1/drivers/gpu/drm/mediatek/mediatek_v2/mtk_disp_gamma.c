	...
	struct cmdq_pkt *handle, void *data)
{
	...
	struct mtk_disp_gamma *gamma = comp_to_gamma(comp);
	struct mtk_disp_gamma_primary *primary_data = gamma->primary_data;
	int sram_table_config_sel = 0, sram_table_out_sel = 0;
	unsigned int read_value = 0;

	CRTC_MMP_MARK(0, aal_ess20_gamma, comp->id, 0);
	mutex_lock(&gamma->primary_data->global_lock);

	read_value = readl(comp->regs + DISP_GAMMA_SHADOW_SRAM);
	sram_table_config_sel = (read_value >> 1) & 0x1;
	sram_table_out_sel = read_value  & 0x1;
	DDPINFO("gamma: SRAM table_config_sel %d, table_config_sel, %d, SRAM table_out_sel %d, table_out_sel, %d",
		sram_table_config_sel, primary_data->table_config_sel, sram_table_out_sel, primary_data->table_out_sel);
	if (sram_table_out_sel != primary_data->table_out_sel) {
		DDPINFO("gamma: SRAM config %d != %d config", sram_table_config_sel,
			primary_data->table_config_sel);
			mutex_unlock(&gamma->primary_data->global_lock);
			return -EFAULT;
	}
