...
int mtk_drm_dump_crtc_wk_lock_ref(unsigned int crtc_idx)
{
	...
	return (atomic_read(&mtk_crtc->wk_lock_ref) - 1);
}
EXPORT_SYMBOL(mtk_drm_dump_crtc_wk_lock_ref);
...
#ifdef OPLUS_FEATURE_DISPLAY
	else if (strncmp(opt, "read_ddic:", strlen("read_ddic:")) == 0) {
		int ret_num, ret;
		char ret_val[10];
		unsigned int addr;
		u8 ddic_reg;
		ret = sscanf(opt, "read_ddic:0x%x, %d", &addr, &ret_num);
		if (ret < 0) {
			DDPPR_ERR("%d error to set disp_cm_set %s\n", __LINE__, opt);
			return;
		}
		ddic_reg = (u8)addr;
		DDPMSG("read_ddic:0x%x,%d\n", ddic_reg, ret_num);
		oplus_mtk_read_ddic_v2(ddic_reg, ret_num, ret_val);
...
#ifdef OPLUS_FEATURE_DISPLAY
	oplus_display_set_logger_en(true);
#endif
