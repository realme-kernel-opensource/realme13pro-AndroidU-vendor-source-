...
static bool disp_c3d_write_sram(struct mtk_ddp_comp *comp, int cmd_type)
{
	...
	case C3D_PREPARE:
		...
		if (async == false) {
			...
			C3DFLOW_LOG("mtk_drm_idle_async_flush_cust\n");
			ret = mtk_drm_idle_async_flush_cust(crtc, comp->id,
						cmdq_handle, false, NULL);
			if (ret < 0) {
				cmdq_pkt_flush(cmdq_handle);
				C3DFLOW_LOG("failed of async flush\n");
	...
	if (c3dBinNum == 17) {
		if (!c3d_data->is_right_pipe) {
			if (copy_from_user(&primary_data->c3d_reg_17bin,
				...
				C3DFLOW_LOG("4910: %x 4911: %x 4912: %x\n",
					primary_data->c3d_sram_cfg[4910],
					primary_data->c3d_sram_cfg[4911],
