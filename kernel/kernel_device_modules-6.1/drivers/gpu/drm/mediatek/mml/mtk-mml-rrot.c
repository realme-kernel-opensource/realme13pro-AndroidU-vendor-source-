...
#define RROT_STASH_ENABLE	0x43000000
#define RROT_STASH_DISABLE	0x80000000

int rrot_stash_en = 1;
module_param(rrot_stash_en, int, 0644);
	...
	struct cmdq_pkt *pkt, const phys_addr_t base_pa)
{
	...
	cmdq_pkt_write(pkt, NULL, base_pa + RROT_PREFETCH_CONTROL_0, RROT_STASH_DISABLE, U32_MAX);
	...
	u32 format, u32 width, u32 height, u32 rot)
{
	...
	if (rrot_stash_en) {
		cmdq_pkt_write(pkt, NULL, base_pa + RROT_PREFETCH_CONTROL_0,
			RROT_STASH_ENABLE, U32_MAX);
