...
#include "ac158_p_7_a0001_cmd_panel.h"
#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac158_p_7_a0001_data_hw_roundedpattern.h"
...
extern unsigned int oplus_bl_demura_dbv_poweron;
...
//	.move_esd_readdate_back = 1,
	...
	.lcm_esd_check_table[2] = {
		.cmd = 0x03, .count = 1, .para_list[0] = 0x00,
	},
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	pr_info("level=%d, bl_demura_mode=%d, oplus_bl_demura_dbv_poweron=%d\n", level, bl_demura_mode, oplus_bl_demura_dbv_poweron);
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_info("%s: p_7_a0001 lcm ctx->prepared %d\n", __func__, ctx->prepared);
...
static int oplus_display_panel_set_demura_bl(void *dsi, dcs_write_gce_pack cb, void *handle, int bl_demura_mode)
{
	...
	switch (bl_demura_mode) {
		...
		case OPLUS_DEMURA_DBV_MODE3:
			index = sizeof(dsi_demura3_bl) / sizeof(struct LCM_setting_table);
			panel_send_pack_lp_cmd(dsi, dsi_demura3_bl, index, cb, handle);
			break;
		case OPLUS_DEMURA_DBV_MODE4:
			index = sizeof(dsi_demura4_bl) / sizeof(struct LCM_setting_table);
			panel_send_pack_lp_cmd(dsi, dsi_demura4_bl, index, cb, handle);
			break;
		default:
			pr_err("Unsupported mode\n");
			break;
	}
	pr_info("bl_demura_mode=%d, demura_bl_1=%d demura_bl_2=%d demura_bl_3=%d demura_bl_4=%d\n", bl_demura_mode, demura_setting.demura_switch_dvb1,
		demura_setting.demura_switch_dvb2, demura_setting.demura_switch_dvb3, demura_setting.demura_switch_dvb4);
	return 0;
}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM] ac158_p_7_a0001 %s START\n", __func__);
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	pr_info("[LCM] %s- lcm, AC158_P_7_A0001, END\n", __func__);
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac158_p_7_a0001_cmd_panel",
		.owner = THIS_MODULE,
