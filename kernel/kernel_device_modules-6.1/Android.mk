...
# Configure PGO
$(info >>> KERNEL_PGO_FLAG: $(KERNEL_PGO_FLAG))
ifeq ($(shell echo $$KERNEL_PGO_FLAG),1)
  PGO_KBUILD_ARG := --pgo=inst
else ifeq ($(shell echo $$KERNEL_PGO_FLAG),2)
  PGO_KBUILD_ARG := --pgo=pgo
else
  PGO_KBUILD_ARG := --pgo=none
endif
$(info >>> PGO_KBUILD_ARG: $(PGO_KBUILD_ARG))
...
$(KERNEL_ZIMAGE_OUT): $(GEN_KERNEL_BUILD_CONFIG) $(KERNEL_MAKE_DEPENDENCIES) | kernel-outputmakefile
	$(hide) mkdir -p $(dir $@)
	$(hide) cd kernel && $(PRIVATE_KERNEL_MAKE_OPTION)  CC_WRAPPER=$(PRIVATE_CC_WRAPPER) SKIP_MRPROPER=1 BUILD_CONFIG=$(PRIVATE_KERNEL_BUILD_CONFIG) OUT_DIR=$(PRIVATE_KERNEL_OUT) DIST_DIR=$(PRIVATE_DIST_DIR) $(PRIVATE_KERNEL_BUILD_SCRIPT) && cd ..
	$(hide) $(call fixup-kernel-cmd-file,$(KERNEL_OUT)/arch/$(KERNEL_TARGET_ARCH)/boot/compressed/.piggy.xzkern.cmd)
else
$(KERNEL_ZIMAGE_OUT): PRIVATE_OPLUS_FEATURES := $(shell export | grep -e "^declare -x OPLUS_FEATURE_BSP_" | sed 's/declare -x //g')
$(KERNEL_ZIMAGE_OUT): PRIVATE_BAZEL_EXPORT_ENV := BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN=1 DEFCONFIG_OVERLAYS="$(KERNEL_DEFCONFIG_OVERLAYS)" KERNEL_VERSION=$(LINUX_KERNEL_VERSION) OPLUS_DEFCONFIG_OVERLAYS=$(COMPILE_PLATFORM) OPLUS_FEATURES="$(PRIVATE_OPLUS_FEATURES)"
...
$(KERNEL_ZIMAGE_OUT): $(KERNEL_MAKE_DEPENDENCIES)
	...
	$(hide) cd kernel && export $(PRIVATE_BAZEL_EXPORT_ENV) && tools/bazel --output_root=$(abspath $(PRIVATE_BAZEL_BUILD_OUT)) --output_base=$(abspath $(PRIVATE_BAZEL_BUILD_OUT))/bazel/output_user_root/output_base build $(PRIVATE_BAZEL_BUILD_FLAG) $(PGO_KBUILD_ARG) $(PRIVATE_BAZEL_BUILD_GOAL)
	$(hide) cd kernel && export $(PRIVATE_BAZEL_EXPORT_ENV) && tools/bazel --output_root=$(abspath $(PRIVATE_BAZEL_BUILD_OUT)) --output_base=$(abspath $(PRIVATE_BAZEL_BUILD_OUT))/bazel/output_user_root/output_base run $(PRIVATE_BAZEL_BUILD_FLAG) $(PGO_KBUILD_ARG) --nokmi_symbol_list_violations_check $(PRIVATE_BAZEL_DIST_GOAL) -- --dist_dir=$(abspath $(PRIVATE_BAZEL_DIST_OUT))
ifeq (true,$(strip $(KERNEL_CHECK_ABI)))
