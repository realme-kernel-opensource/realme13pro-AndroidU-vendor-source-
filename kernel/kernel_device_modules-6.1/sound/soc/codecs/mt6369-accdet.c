...
/* add for fix headset hook key up event lose issues */
	if (accdet->cable_type == HEADSET_MIC) {
		 ...
		 */
		accdet_clear_bit(ACCDET_SW_EN_ADDR, ACCDET_SW_EN_SFT);
		disable_accdet();
	}
...
static u32 config_moisture_detect_2_1_1(void)
{
	...
	if (eintvth == 0) {
		...
		/* Only the HP_EINT using 3.0V AVDD*/
		if (accdet_dts.moisture_vref2_modify) {
			accdet_write(0x2526, 0x6);
		}
	}
	return 0;
...
static int accdet_get_dts_data(void)
{
	...
	/* 2024/6/29, add for modify vref2 from 900mv to 990mv*/
	/* Only the HP_EINT using 3.0V AVDD*/
 	ret = of_property_read_u32(node,
			"moisture-vref2-modify", &accdet_dts.moisture_vref2_modify);
	if (ret) {
		/* Keep vref2 unchanged */
 		pr_info("%s: read moisture-vref2-modify fail\n", __func__);
		accdet_dts.moisture_vref2_modify = 0;
	}
...
static void accdet_init_once(void)
{
	...
	if (accdet_dts.headset_eint0_disable) {
		accdet_clear_bit(ACCDET_EINT0_INVERTER_SW_EN_ADDR, ACCDET_EINT0_INVERTER_SW_EN_SFT);
	}
	pr_info("%s done!\n", __func__);
}
