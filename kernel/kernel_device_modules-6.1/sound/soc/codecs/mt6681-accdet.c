...
static void send_accdet_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
	case HEADSET_MIC:
		accdet->oplus_accdet_plug_state = status? CABLE_PLUGIN : CABLE_PLUGOUT;
		break;
	case LINE_OUT_DEVICE:
		accdet->oplus_accdet_plug_state = status? CABLE_INVALID : CABLE_PLUGOUT;
		break;
	default:
		accdet->oplus_accdet_plug_state = CABLE_INVALID;
	}
...
// add for check and reset MT6681_TOP_INT_CON0 bit7 for headset can't be detected issue
#define TOP_INT_CON0_MAX_RETRY_COUNT (3)
bool mtk_accdet_reg_check_and_reset(u16 addr, u8 shift) {
	u8 irq_state = 0;
	int i = 0;

	irq_state = pmic_read_mbit(addr, shift, 0x01);

	while ((irq_state != 0x01) && (i < TOP_INT_CON0_MAX_RETRY_COUNT)) {
		pmic_write_set(addr, shift);
...
bool mtk_accdet_irq_check_and_set(void) {
	bool reg_check = false;
	reg_check = mtk_accdet_reg_check_and_reset(MT6681_TOP_INT_CON0, 7);
	reg_check &= mtk_accdet_reg_check_and_reset(MT6681_AUD_TOP_INT_CON0, 5);

	return reg_check;
}
EXPORT_SYMBOL(mtk_accdet_irq_check_and_set);
#endif /* CONFIG_OPLUS_MTK_ACCDET_CHECK_RESET */

#if IS_ENABLED(CONFIG_OPLUS_MTK_HEADSET_RECHECK)
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
//add for dump accdet register after when no irq received
static void oplus_mini_dump_register(void)
{
	int addr = 0, st_addr = 0, end_addr = 0, idx = 0, log_size = 0;
	char accdet_fb_buf[512];

	log_size +=
	snprintf(accdet_fb_buf, sizeof(accdet_fb_buf),
		"accdet regs list: (0x%x)=0x%x, (0x%x)=0x%x, (0x%x)=0x%x, (0x%x)=0x%x, (0x%x)=0x%x, (0x%x)=0x%x, ",
		MT6681_AUD_TOP_INT_CON0, pmic_read(MT6681_AUD_TOP_INT_CON0),
		MT6681_ACCDET_SW_EN_ADDR, pmic_read(MT6681_ACCDET_SW_EN_ADDR),
		MT6681_ACCDET_CMP_PWM_EN_ADDR, pmic_read(MT6681_ACCDET_CMP_PWM_EN_ADDR),
		MT6681_ACCDET_IRQ_ADDR, pmic_read(MT6681_ACCDET_IRQ_ADDR),
		MT6681_ACCDET_DA_STABLE_ADDR, pmic_read(MT6681_ACCDET_DA_STABLE_ADDR),
		MT6681_ACCDET_HWMODE_EN_ADDR, pmic_read(MT6681_ACCDET_HWMODE_EN_ADDR));
	log_size +=
	snprintf(accdet_fb_buf + log_size, (sizeof(accdet_fb_buf) - log_size),
	...
	log_size +=
	snprintf(accdet_fb_buf + log_size, (sizeof(accdet_fb_buf) - log_size),
		"start addr(0x%x) end addr(0x%x), regs list: ",
		MT6681_RG_AUDPWDBMICBIAS0_ADDR, MT6681_RG_ACCDETSPARE_ADDR);

	st_addr = MT6681_RG_AUDPWDBMICBIAS0_ADDR;
	end_addr = MT6681_RG_ACCDETSPARE_ADDR;
	for (addr = st_addr; addr <= end_addr; addr += 4) {
		idx = addr;
		log_size += snprintf(accdet_fb_buf + log_size, (sizeof(accdet_fb_buf) - log_size),
			"0x%x, 0x%x, 0x%x, 0x%x",
			pmic_read(idx), pmic_read(idx+1), pmic_read(idx+2), pmic_read(idx+3));
	}
	if (log_size < 0) {
		pr_notice("sprintf failed\n");
		return;
	...
	mm_fb_audio_fatal_delay(10047, MM_FB_KEY_RATELIMIT_5MIN,
							FEEDBACK_DELAY_60S, accdet_fb_buf);
}
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */

/* Add for check cable state after plug in/out 1s and force accdet irq. */
u32 mtk_get_accdet_plug_state(void) {
	pr_info("%s accdet->oplus_accdet_plug_state = %d.\r",
			__func__, accdet->oplus_accdet_plug_state);
	return accdet->oplus_accdet_plug_state;
}
EXPORT_SYMBOL(mtk_get_accdet_plug_state);

int oplus_force_eint_handler(int plug_state)
{
	//int ret = 0;
	pr_info("%s called to force check %s.\n",
			__func__, plug_state? "plug in" : "plug out");

	#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	//add for dump accdet register after when no irq received
	oplus_mini_dump_register();
	#endif
	//accdet->cur_eint_state = plug_state;
	//if (accdet->cur_eint_state == EINT_PIN_PLUG_IN) {
	//	if (accdet_dts.moisture_detect_mode != 0x5) {
	//		mod_timer(&micbias_timer,
	//			jiffies + MICBIAS_DISABLE_TIMER);
...
EXPORT_SYMBOL(oplus_force_eint_handler);
#endif /* CONFIG_OPLUS_MTK_HEADSET_RECHECK */
...
#if IS_ENABLED(CONFIG_OPLUS_MTK_HEADSET_RECHECK)
/* Add for check cable state after plug in/out 1s and force accdet irq. */
	accdet->oplus_accdet_plug_state = CABLE_PLUGOUT;
#endif
