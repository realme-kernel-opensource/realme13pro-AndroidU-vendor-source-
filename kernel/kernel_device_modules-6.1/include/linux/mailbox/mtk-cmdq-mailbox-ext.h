...
typedef bool (*util_irq_enable_mbox)(struct device *dev);
typedef bool (*util_irq_disable_mbox)(struct device *dev);

struct cmdq_util_controller_fp {
	util_track_ctrl track_ctrl;
	util_thread_ddr_module thread_ddr_module;
	util_check_tf check_tf;
	util_irq_enable_mbox irq_enable_mbox;
	util_irq_disable_mbox irq_disable_mbox;
};
