...
uint8_t mt6639_dumpCbtopReg(struct ADAPTER *ad)
{
	...
	if (fgIsBusAccessFailed || fgIsMcuOff) {
		...
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[0]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[1]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[2]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[3]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[4]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[5]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[6]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[7]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[8]) != 0)
			return FALSE;
		read_addr += 4;
		if (connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI,
				CONNV3_DRV_TYPE_BT,
				read_addr, &u4Values[9]) != 0)
			return FALSE;
		read_addr += 4;
		DBGLOG(HAL, INFO,
...
void mt6639_DumpBusHangCr(struct ADAPTER *ad)
{
	...
	DBGLOG(HAL, INFO, "Phase1: Trigger Wi-Fi dump via PCIe.\n");
	...
	DBGLOG(HAL, INFO, "Phase2: Trigger PCIe Scan Dump.\n");
start_dump_via_bt:
#if IS_ENABLED(CFG_MTK_WIFI_CONNV3_SUPPORT)
	/* Notify BT to start */
