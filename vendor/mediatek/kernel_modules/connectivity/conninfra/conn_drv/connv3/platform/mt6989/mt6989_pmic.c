...
#define MT6376_REG_SIZE 8
	...
	if (connsys_pmic_ecid_ready == false && mtk_ecid_dump_enable()) {
		ret = snprintf(connsys_pmic_ecid, CHIP_ECIP_INFO_LENGTH, "MT6376P_ECID:%02X %02X\n", buffer[26], buffer[27]);
		if (ret <= 0)
			pr_notice("%s snprintf fail", __func__);
		else
			connsys_pmic_ecid_ready = true;
	}
	if (connsys_chip_ecid_ready == false && mtk_ecid_dump_enable()) {
		ret = snprintf(connsys_chip_ecid, CHIP_ECIP_INFO_LENGTH, "MT6639AEW_ECID:%02X %02X %02X %02X %02X %02X %02X %02X\n",
