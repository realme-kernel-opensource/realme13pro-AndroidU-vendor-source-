...
int conninfra_core_pmic_event_cb(unsigned int id, unsigned int event)
{
	...
	unsigned int oc_reset_switch = 0;
	int ret;
	...
	if (NULL == conf) {
		...
		oc_reset_switch = conf->oc_reset_switch;
	}
	...
	if (oc_reset_switch) {
		pr_info("[%s] Set oc_reset_switch on, trigger whole chip reset.", __func__);
		conninfra_trigger_whole_chip_rst(CONNDRV_TYPE_CONNINFRA, "OC");
	} else {
		pr_info("[%s] Set oc_reset_switch off, do nothing.", __func__);
	}
