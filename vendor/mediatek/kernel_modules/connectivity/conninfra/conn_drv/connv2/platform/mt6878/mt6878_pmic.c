...
static struct notifier_block vcn33_1_nb;
static struct notifier_block vcn33_2_nb;
...
static int consys_vcn33_1_oc_notify(struct notifier_block*, unsigned long, void*);
static int consys_vcn33_2_oc_notify(struct notifier_block*, unsigned long, void*);
...
int consys_plt_pmic_get_from_dts_mt6878(struct platform_device *pdev, struct conninfra_dev_cb* dev_cb)
{
	...
	if (IS_ERR(reg_VCN33_1)) {
		...
		vcn33_1_nb.notifier_call = consys_vcn33_1_oc_notify;
		ret = devm_regulator_register_notifier(reg_VCN33_1, &vcn33_1_nb);
		if (ret)
			pr_info("VCN33_1 regulator notifier request failed\n");
	}
	...
	if (IS_ERR(reg_VCN33_2)) {
		...
		vcn33_2_nb.notifier_call = consys_vcn33_2_oc_notify;
		ret = devm_regulator_register_notifier(reg_VCN33_2, &vcn33_2_nb);
		if (ret)
			pr_info("VCN33_2 regulator notifier request failed\n");
	}
...
static int consys_vcn33_1_oc_notify(struct notifier_block *nb, unsigned long event,
				  void *unused)
{
	static int oc_counter = 0;
	static int oc_dump = 0;
...
static int consys_vcn33_2_oc_notify(struct notifier_block *nb, unsigned long event,
				  void *unused)
{
	static int oc_counter = 0;
	static int oc_dump = 0;
