				  ...
				  struct v4l2_buffer *buf)
{
	...
	if ((node->desc->id == MTK_IMGSYS_VIDEO_NODE_ID_CTRLMETA_OUT) ||
		...
		/*workaround for the planes in the v4l2 buf
		 *not clone from user space to kernel space
		 */
		//dev_buf->dataofst = buf->m.planes[0].reserved[2];
		if (buf->length == 0) {
			dev_buf->dataofst = 309464;
            pr_info("imgsys--dmaport-%s-%d-buf length(%d)\n",
				node->desc->name, node->desc->id, buf->length);
		}
		else {
			dev_buf->dataofst = buf->m.planes[0].reserved[2];
	}
