...
void set_mirror_flip(struct subdrv_ctx *ctx, u8 image_mirror)
{
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].ae_ctrl_support ==
		IMGSENSOR_AE_CONTROL_SUPPORT_SENSING_MODE) {
		if (ctx->s_ctx.reg_addr_aov_mode_mirror_flip == PARAM_UNDEFINED) {
			DRV_LOG(ctx, "sensor no support\n");
			return;
		}
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].ae_ctrl_support ==
		IMGSENSOR_AE_CONTROL_SUPPORT_SENSING_MODE)
		subdrv_i2c_wr_u8(ctx, ctx->s_ctx.reg_addr_aov_mode_mirror_flip, itemp);
	else
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, u32 *margin)
{
	...
	*margin = ctx->s_ctx.mode[scenario_id].exposure_margin ? ctx->s_ctx.mode[scenario_id].exposure_margin : ctx->s_ctx.exposure_margin;
	DRV_LOG(ctx, "mode :%d, current_margin:%d\n",scenario_id, *margin);
}
