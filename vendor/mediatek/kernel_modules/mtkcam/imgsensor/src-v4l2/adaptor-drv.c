...
static int imgsensor_stop_streaming(struct adaptor_ctx *ctx)
{
	...
	if (!(ctx->subctx.s_ctx.aov_sensor_support && ctx->subctx.s_ctx.mode[ctx->subctx.current_scenario_id].aov_mode)) {
		subdrv_call(ctx, feature_control,
			SENSOR_FEATURE_SET_TEST_PATTERN,
			para.u8, &len);
	}
...
static int imgsensor_probe(struct i2c_client *client)
{
	...
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
	dev_info(dev,
		"get platform device by device node\n");
	pPdev = of_find_device_by_node(platform_node);
	res = platform_get_resource_byname(pPdev, IORESOURCE_MEM, "res_base");
	ctx->subctx.reg_spm = of_iomap(platform_node, 2);
	if (IS_ERR(ctx->subctx.reg_spm))
		return PTR_ERR(ctx->subctx.reg_spm);

	dev_info(dev,
		"get spm base address\n");
	res = platform_get_resource_byname(pPdev, IORESOURCE_MEM, "res_spare_base");
	ctx->subctx.reg_spmspare = of_iomap(platform_node, 3);
	if (IS_ERR(ctx->subctx.reg_spmspare))
		return PTR_ERR(ctx->subctx.reg_spmspare);

	dev_info(dev, "probe spmreg:0x%p, spmspare:0x%p\n", ctx->subctx.reg_spm, ctx->subctx.reg_spmspare);

	dev_info(dev,
		"get spm spare base address\n");
	#endif /* OPLUS_FEATURE_CAMERA_COMMON */
