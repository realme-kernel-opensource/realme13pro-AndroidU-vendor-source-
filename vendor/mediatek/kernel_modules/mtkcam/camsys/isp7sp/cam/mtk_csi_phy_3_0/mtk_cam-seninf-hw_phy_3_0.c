...
static int csirx_phyA_setting(struct seninf_ctx *ctx)
{
	...
	if (!ctx->is_cphy) { //Dphy
		...
		if (ctx->is_4d1c) {
			...
			// main sensor csi_port = 2
			if (ctx->port == CSI_PORT_2) {
				dev_info(ctx->dev, "main sensor: csi_port = %d\n", ctx->port);
				...
				// SENINF_WRITE_REG(baseB, CDPHY_RX_ANA_5, 0x157);
				// dev_info(ctx->dev, "CDPHY_RX_ANA_5:(0x%08x)\n", SENINF_READ_REG(baseA, CDPHY_RX_ANA_5));
				// if (!strcasecmp(_seninf_ops->iomem_ver, MT6989_IOMOM_VERSIONS)) {
				// 	SENINF_BITS(baseA, CDPHY_RX_ANA_5,
				// 		RG_CSI0_CDPHY_EQ_IS, 0x2);
				// 	ret = SENINF_READ_BITS(baseA, CDPHY_RX_ANA_8, RG_CSI0_RESERVE);
				// 	SENINF_BITS(baseA, CDPHY_RX_ANA_8,
				...
				// 	return -EINVAL;
				// }
			} else if (ctx->port == CSI_PORT_0) {
				//4.EQ_SR0 = 0111
				dev_info(ctx->dev, "tele sensor: csi_port = %d\n", ctx->port);
				...
				dev_info(ctx->dev, "other sensor: csi_port = %d\n", ctx->port);
				if (data_rate < 2500000000) {
					SENINF_WRITE_REG(baseA, CDPHY_RX_ANA_5, 0x55);
					SENINF_WRITE_REG(baseB, CDPHY_RX_ANA_5, 0x55);
				} else {
