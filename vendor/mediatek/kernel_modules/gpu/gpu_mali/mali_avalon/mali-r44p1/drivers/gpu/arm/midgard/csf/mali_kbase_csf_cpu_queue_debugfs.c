					...
					struct base_csf_notification *req)
{
	if (atomic_cmpxchg(&kctx->csf.cpu_queue.dump_req_status,
		...
		BASE_CSF_CPU_QUEUE_DUMP_ISSUED) {
		dev_info(kctx->kbdev->dev,"%s<%d>[dump_req_status] Status failed\n",__func__, __LINE__);
		return false;
	}
	dev_info(kctx->kbdev->dev,"%s<%d>[dump_req_status] PENDING(%d)\n",__func__, __LINE__, BASE_CSF_CPU_QUEUE_DUMP_PENDING);
	mtk_log_critical_exception(kctx->kbdev, true,
		"[dump_req_status] PENDING(%d) ", BASE_CSF_CPU_QUEUE_DUMP_PENDING);
