...
int disable_ux_load = 0;
EXPORT_SYMBOL(disable_ux_load);
...
#define DISABLE_UX_LOAD_SHIFT (4)
#define DISABLE_UX_LOAD_MASK  (1 << DISABLE_UX_LOAD_SHIFT)

#define EAS_OPT_MASK (0x0000000F)
		...
		size_t count, loff_t *ppos)
{
	...
	disable_ux_load = (val & DISABLE_UX_LOAD_MASK) >> DISABLE_UX_LOAD_SHIFT;
	val = val & EAS_OPT_MASK;
	switch (val) {
	case 0:
		eas_opt_enable = 0;
		...
		size_t count, loff_t *ppos)
{
	...
	len = snprintf(buffer, sizeof(buffer), "eas_enable=%d eas_debug_enable=%d dis_ux_load=%d\n", eas_opt_enable, eas_opt_debug_enable, disable_ux_load);
