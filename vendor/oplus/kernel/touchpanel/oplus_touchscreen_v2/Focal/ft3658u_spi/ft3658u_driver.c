...
static int fts_enable_game_mode(struct chip_data_ft3658u *ts_data, bool enable)
{
	...
	if (enable) {
		...
		fts_write_reg(FT3658U_REG_REPORT_RATE_2K, 0x01);
	} else {
		report_rate = FTS_120HZ_REPORT_RATE;
		fts_write_reg(FT3658U_REG_REPORT_RATE_2K, 0x00);
	}
	/* TPD_INFO("MODE_GAME, write 0x8B|23=0x%x", ts_data->ctrl_reg_state); */
	return fts_write_reg(FTS_REG_GAME_MODE_EN, report_rate);
}
...
static int fts_set_high_frame_rate(void *chip_data, int level, int time)
{
	...
	if (level > 0) {
		...
		ret = fts_write_reg(FTS_REG_GAME_MODE_EN, 2);
		if (ret < 0) {
			return ret;
		}
...
static void fts_rate_white_list_ctrl(void *chip_data, int value)
{
	...
	case FTS_WRITE_RATE_240:
		send_value = 2; /*IC does not support reporting rate*/
		break;
	default:
