 ...
 *============================================================================
 ****************************************************************************/
#include "omegas3widemipiraw_Sensor.h"

#define OMEGAS3WIDE_EEPROM_READ_ID	0xA3
#define OMEGAS3WIDE_EEPROM_WRITE_ID	0xA2
#define OMEGAS3WIDE_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "omegas3wide_camera_sensor"
...
static int omegas3wide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int omegas3wide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int omegas3wide_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int omegas3wide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int omegas3wide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int omegas3wide_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct eeprom_map_info omegas3wide_eeprom_info[] = {
	...
	{ EEPROM_META_MODULE_SN, 0x00B0, 0x00C8, 0x00C9, 23, true },
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, omegas3wide_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, omegas3wide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, omegas3wide_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, omegas3wide_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, omegas3wide_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, omegas3wide_get_min_shutter_by_scenario_adapter},
};

static struct eeprom_info_struct eeprom_info[] = {
	{
		.header_id = 0x016D0006,
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = omegas3wide_preview_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_preview_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 280800000,
...

	{
		...
		.mode_setting_table = omegas3wide_capture_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_capture_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 280800000,
...

	{
		...
		.mode_setting_table = omegas3wide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 280800000,
...

	{
		...
		.mode_setting_table = omegas3wide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 288000000,
...

	{
		...
		.mode_setting_table = omegas3wide_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_slim_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 280800000,
...

	{
		...
		.mode_setting_table = omegas3wide_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 273600000,
...

	{
		...
		.mode_setting_table = omegas3wide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 144000000,
...

	{
		...
		.mode_setting_table = omegas3wide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 280800000,
...

	{
		...
		.mode_setting_table = omegas3wide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 211200000,
...

	{
		...
		.mode_setting_table = omegas3wide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 144000000,
	...
	{/*Reg H-3_QBIN-V2H2 FHD 2048x1152_480FPS */
		...
		.mode_setting_table = omegas3wide_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom6_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 288000000,
	...
	{ /*reg_K 1632*1224@24*/
		...
		.mode_setting_table = omegas3wide_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(omegas3wide_custom7_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 199200000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = OMEGAS3WIDE_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = omegas3wide_ana_gain_table,
	.ana_gain_table_size = sizeof(omegas3wide_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 1,
	...
	.init_setting_table = omegas3wide_init_setting,
	.init_setting_len = ARRAY_SIZE(omegas3wide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_DVDD, 1184000, 0},
	{HW_ID_DOVDD, 1800000, 3},
	{HW_ID_MCLK_DRIVING_CURRENT, 4, 6},
	{HW_ID_RST, 1, 2},
};

const struct subdrv_entry omegas3wide_mipi_raw_entry = {
	.name = "omegas3wide_mipi_raw",
	.id = OMEGAS3WIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_omegas3wide_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != omegas3wide_eeprom_info[meta_id].meta)
		return -1;

	if (size != omegas3wide_eeprom_info[meta_id].size)
		return -1;

	addr = omegas3wide_eeprom_info[meta_id].start;
	readsize = omegas3wide_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, OMEGAS3WIDE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
		...
		kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, OMEGAS3WIDE_EEPROM_WRITE_ID >> 1,
			addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, OMEGAS3WIDE_EEPROM_READ_ID >> 1, reg, 0xA3);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, OMEGAS3WIDE_EEPROM_READ_ID >> 1, reg, 0xA2);
	}
			...
			ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == OMEGAS3WIDE_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
			&& (data_base == OMEGAS3WIDE_STEREO_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == OMEGAS3WIDE_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
			&& (data_base == OMEGAS3WIDE_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+1),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+2),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+3),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+4),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+5),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+6),
					read_cmos_eeprom_8(ctx, OMEGAS3WIDE_AESYNC_START_ADDR+7));
		...
		LOG_INF("omegas3 write_Module_data pStereodata is null\n");
		return -1;
	}
	return ret;
}
...
static int omegas3wide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_omegas3wide_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
					BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, OMEGAS3WIDE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "omegas3wide read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "omegas3wide read_otp_info end\n");
}
...
void omegas3wide_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
...
int omegas3wide_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	omegas3wide_get_min_shutter_by_scenario(ctx,
