...
#define PFX "CAM_CAL_OMEGAS3MAIN"
...
static unsigned int do_single_lsc_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x016B012B, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001070, 0x0000074C, do_single_lsc_omegas3main},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_omegas3main}, //Start address, block size is useless
		{0x00000001, 0x000017D0, 0x00000A3A, do_pdaf_omegas3main},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_omegas3main}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT omegas3main_op_eeprom = {
	.name = "omegas3main_op_eeprom",
...
/*
unsigned int do_pdaf_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#define OMEGAS3MAIN_HVBIN_PDAF_PROC1_SIZE  (0x1BA0-0x17D0) //976      0x3D0
#define OMEGAS3MAIN_HVBIN_PDAF_PROC2_SIZE  (0x2208-0x1BB0) //1624     2600  0xA28
#define OMEGAS3MAIN_HVBIN_PDAF_PROC1_ADDR  (0x17D0)
#define OMEGAS3MAIN_HVBIN_PDAF_PROC2_ADDR  (0x1BB0)

//partial PD
#define OMEGAS3MAIN_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define OMEGAS3MAIN_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define OMEGAS3MAIN_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define OMEGAS3MAIN_PARTIAL_PD_PROC2_ADDR  (0x35A0)
unsigned int do_pdaf_omegas3main(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, OMEGAS3MAIN_HVBIN_PDAF_PROC1_ADDR, OMEGAS3MAIN_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, OMEGAS3MAIN_HVBIN_PDAF_PROC2_ADDR, OMEGAS3MAIN_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, OMEGAS3MAIN_PARTIAL_PD_PROC1_ADDR, OMEGAS3MAIN_PARTIAL_PD_PROC1_SIZE);
	debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, OMEGAS3MAIN_PARTIAL_PD_PROC2_ADDR, OMEGAS3MAIN_PARTIAL_PD_PROC2_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			OMEGAS3MAIN_PARTIAL_PD_PROC1_ADDR + OMEGAS3MAIN_PARTIAL_PD_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += OMEGAS3MAIN_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				OMEGAS3MAIN_PARTIAL_PD_PROC2_ADDR + OMEGAS3MAIN_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += OMEGAS3MAIN_PARTIAL_PD_PROC2_SIZE;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			OMEGAS3MAIN_HVBIN_PDAF_PROC1_ADDR + OMEGAS3MAIN_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += OMEGAS3MAIN_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				OMEGAS3MAIN_HVBIN_PDAF_PROC2_ADDR + OMEGAS3MAIN_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += OMEGAS3MAIN_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
