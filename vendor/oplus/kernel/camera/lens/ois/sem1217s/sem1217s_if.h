#ifndef __SEM1217S_IF_H__
#define __SEM1217S_IF_H__

#define SEM1217S_FLASH_SLV_ADDR 0x38 //0x50-->0x   //0x38
#define SEM1217S_SLV_ADDR 0x61

extern struct sem1217s_device *sem1217s;
...
uint8_t I2C_OIS_8bit__read(uint32_t addr)
{
	...
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_u8(i2c_client, SEM1217S_SLV_ADDR, addr, &data);
	if (ret < 0) {
...
uint16_t I2C_OIS_16bit__read(uint32_t addr)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_u16(i2c_client, SEM1217S_SLV_ADDR, addr, &data);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, addr);
	}
	return data;
...
uint32_t I2C_OIS_32bit__read(uint32_t addr)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_p8(i2c_client, SEM1217S_SLV_ADDR, addr, data, 4);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, addr);
	}
...
int I2C_OIS_8bit_write(uint32_t addr, uint8_t data)
{
	...
	ret = adaptor_i2c_wr_u8(i2c_client, SEM1217S_SLV_ADDR, addr, data);
	if (ret < 0) {
...
int I2C_OIS_16bit_write(uint32_t addr, uint16_t data)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_wr_u16(i2c_client, SEM1217S_SLV_ADDR, addr, data);
	if (ret < 0) {
		pr_err("%s failed addr[%x] data[%x]", __func__, addr, data);
	}
	return ret;
...
int I2C_OIS_block_write(void *register_data, int size)
{
	int ret;
	uint16_t reg_addr;
	...
	ret = adaptor_i2c_wr_p8(i2c_client, SEM1217S_SLV_ADDR, reg_addr,
				&data[2], size - 2);
	if (ret < 0) {
...
uint8_t I2C_FW_8bit__read(uint32_t addr)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_u8(i2c_client, SEM1217S_FLASH_SLV_ADDR, addr,
				&data);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, addr);
	}
...
uint16_t I2C_FM_16bit__read(uint32_t addr)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_u16(i2c_client, SEM1217S_FLASH_SLV_ADDR, addr, &data);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, addr);
	}
	return data;
...
uint32_t I2C_FM_32bit__read(uint32_t addr)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_rd_p8(i2c_client, SEM1217S_FLASH_SLV_ADDR, addr, data,
				4);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, addr);
	}
...
int I2C_FM_8bit_write(uint32_t addr, uint8_t data)
{
	...
	ret = adaptor_i2c_wr_u8(i2c_client, SEM1217S_FLASH_SLV_ADDR, addr, data);
	if (ret < 0) {
		pr_err("%s failed addr[%x] data[%x]", __func__, addr, data);
	}
	return ret;
...
int I2C_FM_16bit_write(uint32_t addr, uint16_t data)
{
	...
	struct i2c_client *i2c_client;
	i2c_client = sem1217s->hf_dev.private_data;
	ret = adaptor_i2c_wr_u16(i2c_client, SEM1217S_FLASH_SLV_ADDR, addr,
				 data);
	if (ret < 0) {
		pr_err("%s failed addr[%x] data[%x]", __func__, addr, data);
	}
...
void I2C_FM_block_write(void *register_data, int size)
{
	...
	i2c_client = sem1217s->hf_dev.private_data;
	reg_addr = (uint16_t)data[0] << 8 | data[1];
	ret = adaptor_i2c_wr_p8(i2c_client, SEM1217S_FLASH_SLV_ADDR, reg_addr,
				&data[2], size - 2);
	if (ret < 0) {
		pr_err("%s failed addr[%x]", __func__, reg_addr);
	}
