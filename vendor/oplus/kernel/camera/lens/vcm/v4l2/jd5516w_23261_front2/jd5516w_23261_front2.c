...
// Copyright (c) 2019 MediaTek Inc.
 ...
 #define JD5516W_EEPROM_I2C_SLAVE_ADDR  0xA8
 #define JD5516W_NAME				"jd5516w_23261_front2"
 ...
 //#define JD5516W_SET_VOLTAGE        2804000
 #define JD5516W_EEPROM_VCMID_ADDR  0X000A
 #define JD5516W_EEPROM_VCMID_1     0XDD
 #define JD5516W_EEPROM_VCMID_2     0X41
 /*
...
static struct regval_list jd5516w_init_regs_another[] = {
  	{0x02, 0x01, 0},
  	{0x02, 0x00, 1},
  	{0x02, 0x02, 0},
  	{0x06, 0x40, 0},
...
static int jd5516w_read_i2c_data(struct i2c_client *client, unsigned char *s_data,
					u16 s_size, unsigned char *r_data, u16 r_size, u16 addr)
{
	int ret = 0;
	client->addr = addr >> 1;
	ret = i2c_master_send(client, s_data, s_size);

	if (ret != s_size) {
		pr_info("I2C send failed!, addr = %x", s_data[0]);
		return -1;
	}
	ret = i2c_master_recv(client, r_data, r_size);
	if (ret != r_size) {
...
static int jd5516w_read_i2c_eeprom_data(struct jd5516w_device *jd5516w, unsigned char *data, u16 addr)
{
	struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
	int ret = 0;
	u8 puSendCmd[2] = {(u8)(addr >> 8), (u8)(addr & 0xFF)};
	ret = jd5516w_read_i2c_data(client, puSendCmd, 2, data, 1, JD5516W_EEPROM_I2C_SLAVE_ADDR);
	if (ret < 0) {
		pr_info("I2C read eeprom failed!");
	}
	return ret;
}
...
static int jd5516w_init(struct jd5516w_device *jd5516w)
{
      ...
      struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
      pr_info("%s\n", __func__);
      jd5516w_read_i2c_eeprom_data(jd5516w, &vcmid, JD5516W_EEPROM_VCMID_ADDR);
      pr_info("%s vcmid %d \n", __func__, vcmid);
      client->addr = JD5516W_I2C_SLAVE_ADDR >> 1;
      if (vcmid == JD5516W_EEPROM_VCMID_2) {
        ret = jd5516w_write_array(jd5516w, jd5516w_init_regs_another,
                ARRAY_SIZE(jd5516w_init_regs_another));
      } else {
...
static int jd5516w_power_on(struct jd5516w_device *jd5516w)
{
	...
	/* regulator_set_voltage(jd5516w->vin, JD5516W_SET_VOLTAGE, JD5516W_SET_VOLTAGE);
...
static const struct of_device_id jd5516w_of_table[] = {
  	{ .compatible = "oplus,jd5516w_23261_front2" },
  	{ },
};
