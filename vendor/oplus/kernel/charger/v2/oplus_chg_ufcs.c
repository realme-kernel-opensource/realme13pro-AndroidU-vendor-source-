...
#define UFCS_CHG_FULL_INTERVAL	2500
...
static void oplus_ufcs_switch_check_work(struct work_struct *work)
{
	...
	if (get_client_vote(chip->ufcs_disable_votable, CHG_FULL_VOTER) > 0) {
		...
		delta_time = ktime_sub(entry_time, exit_time);
		duration = (unsigned long long) ktime_to_ms(delta_time);
		if (duration <= UFCS_CHG_FULL_INTERVAL) {
			chg_err("duration=%lldms, Less than 2.5s from last full report, force exit ufcs!!!\n", duration);
			return;
		}
	}
