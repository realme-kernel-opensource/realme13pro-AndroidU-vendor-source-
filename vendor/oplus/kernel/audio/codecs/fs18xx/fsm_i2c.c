...
#include "fsm_sysfs.c"
//#include "fsm_codec.c"
#include "fsm_oplus_pa_manager.c"
...
static int fsm_parse_dts(struct i2c_client *i2c, fsm_dev_t *fsm_dev)
{
	...
	ret = of_property_read_u16(np, "fsm,channel-num", &channel_num);
	if (ret) {
		channel_num = -EINVAL;
		dev_err(&i2c->dev, "get channel-num failed!\n");
	}
	fsm_dev->channel_num = channel_num;
	dev_info(&i2c->dev, "channel-num :%d", fsm_dev->channel_num);
			...
			const struct i2c_device_id *id)
{
...
#else
			devm_gpiod_put(&i2c->dev, gpio_to_desc(fsm_dev->rst_gpio));
#endif
		}
		devm_kfree(&i2c->dev, fsm_dev);
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	...
	fsm_oplus_speaker_pa_init(fsm_dev);
#endif
	if(fsm_dev->id == 0) {
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
call_feedback:
	mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_SMARTPA_ERR,MM_FB_KEY_RATELIMIT_5MIN,FEEDBACK_DELAY_60S, "playload@@foursemi-speaker: %s feedaback error %d", __func__, ret);
	return ret;
#endif
}
...
#endif
{
	...
	fsm_dev = i2c_get_clientdata(i2c);
	if (fsm_dev == NULL) {
...
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_PA_MANAGER)
	fsm_oplus_speaker_pa_deinit(fsm_dev);
#endif
	if (fsm_dev->fsm_wq) {
...
#else
		devm_gpiod_put(&i2c->dev, gpio_to_desc(fsm_dev->irq_gpio));
#endif
	}
	if (gpio_is_valid(fsm_dev->rst_gpio)) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 0, 0))
